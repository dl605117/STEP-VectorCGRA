...
`define VTB_TEST_FAIL(lineno, out, ref, port_name) \
    ...
    $display("- line number    : line %0d in CgraRTL__999b32a09cf5f2fc_test_run_generic_tb.v.cases", lineno); \
...
module CgraRTL__999b32a09cf5f2fc_tb;
  // convention
  logic clk;
  logic reset;
  integer cycle_count;

  logic [0:0] send_to_cpu_pkt__last ;
  logic [177:0] recv_from_cpu_pkt__msg ;
  logic [0:0] recv_from_cpu_pkt__rdy ;
  logic [0:0] recv_from_cpu_pkt__val ;
  logic [177:0] send_to_cpu_pkt__msg ;
  logic [0:0] send_to_cpu_pkt__rdy ;
  logic [0:0] send_to_cpu_pkt__val ;

  task t(
    input logic [0:0] ref_send_to_cpu_pkt__last,
    input logic [177:0] inp_recv_from_cpu_pkt__msg,
    input logic [0:0] ref_recv_from_cpu_pkt__rdy,
    input logic [0:0] inp_recv_from_cpu_pkt__val,
    input logic [177:0] ref_send_to_cpu_pkt__msg,
  ...
  begin
    ...
    #`INTRA_CYCLE_TIME;
    `CHECK(lineno, send_to_cpu_pkt__last, ref_send_to_cpu_pkt__last, "send_to_cpu_pkt__last (send_to_cpu_pkt__last in Verilog)");
...
`else
  CgraRTL__999b32a09cf5f2fc DUT
`endif
  (
    .clk(clk),
    .reset(reset),
    .send_to_cpu_pkt__last(send_to_cpu_pkt__last),
  ...
  initial begin
    ...
    `include "CgraRTL__999b32a09cf5f2fc_test_run_generic_tb.v.cases"
